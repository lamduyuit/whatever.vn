...
# import pwd
# Can chinh lai ham download thiet ke graph, chinh bottom o trang download
def download_financial(stock_id):
	...
	if kqkd_5_cdkt_5.count() > 0:
		for i in kqkd_5_cdkt_5:
			if i.report_name == "KQKD":
				...
				kqkd_5 = kqkd_5[kqkd_5["Year"] == year_now]
				kqkd_5 = kqkd_5.iloc[0, 1:-2] > 0
				if not kqkd_5.all():
					kqkd_5 = 1
				else:
					kqkd_5 = 0
			if i.report_name == "CDKT":
				...
				cdkt_5 = cdkt_5[cdkt_5["Year"] == year_now]
				cdkt_5 = cdkt_5.iloc[1, 1:-2] > 0
				if not cdkt_5.all():
					cdkt_5 = 1
				else:
					cdkt_5 = 0
	if yearly_data.count() > 0 :
		...
		for y_data in yearly_data:
			...
			if y_data.report_name == "LC":
				Lctt_year = change_type_df_y(y_data,n_year)
				if Lctt_year.iloc[1, -1] == 0:
					Lctt_year.drop(str(year_now),axis=1, inplace= True)
...
def change_type_df_q(df,year):
	...
	# df.index = df["Attribute"].cat.codes
...
def get_chart(stock_id):
	...
	for y_data in yearly_data:
		...
		if y_data.report_name == "CDKT":
			...
			# print(cdkt_year.iloc[:10])
			# print(cdkt_year.head())
		if y_data.report_name == "KQKD":
			kqkd_year = change_type_df_y(y_data,n_year)
		if y_data.report_name == "LC":
			Lctt_year = change_type_df_y(y_data,n_year)
	...
	##################################################CHART3######################################################################
	chart3_4_data = cstc_year.ix[[19,21,15,18],1:]
	# chart3_4_data = cstc_year.ix[[19,21,15,18],1:]*100
	chart3_4_data = chart3_4_data.T
	#print(cstc_year.ix[[19,21,15,18],1:])
	...
	cstc_chart5.ix[73:,1:] = cstc_chart5.ix[73:,1:]
	# cstc_chart5.ix[73:,1:] = cstc_chart5.ix[73:,1:]*100
	# print(cstc_chart5.ix[:,1:])
	...
	# chart6_data.ix[133] = cstc_year.ix[50,1:] *100
	#debt ratio
	chart6_data.ix[134] = cstc_year.ix[51,1:]
	# chart6_data.ix[134] = cstc_year.ix[51,1:] *100
